...
//
//
unsigned long long IsJumpCode(unsigned long long ptr)
{
	...
	bool is_jxx = false;
	if (XdvIsJumpCode(XdvGetArchitectureHandle(), ptr, dump, &is_jxx))
	{
...
unsigned long long CodeAndRemarkString(unsigned long long ptr, std::string &str, std::multimap<unsigned long long, unsigned long long> ref_map)
{
	...
	if (ov.size())
	{
		for (size_t i = 0; i < ov.size(); ++i)
		{
			...
			{
				sprintf_s(asm_mn, sizeof(asm_mn), " %s%*c; %I64x, %s", mn, align, ' ', ov[i], symbol);
			}
			else
			{
				sprintf_s(asm_mn, sizeof(asm_mn), " %s%*c; %I64x, %s", mn, align, ' ', ov[i], "<unknown>");
			}
...
EXT_CLASS_COMMAND(WindbgEngine, ut, "", "{;ed,o;ptr;;}" "{entry;b,o;entry;;}")
{
	...
	{
		std::string str;
		CodeAndRemarkString(*it, str, ref_map);
		if (str.size())
		{
			...
			if (fi != ref_map.end())
			{
				...
				{
					Dml("<link section=\"%I64x\">%I64x</link>", range->second, range->second);
					dprintf(" ");
				}
				dprintf("\n");
			}

			Dml("<link name=\"%I64x\"> %s</link>", *it, str.c_str());
			unsigned long long jump_dest = IsJumpCode(*it);
			if (jump_dest)
			{
				dprintf("  ");
				Dml("<link section=\"%I64x\">jump branch</link>", jump_dest);
				dprintf("\n");
			}
			dprintf("\n");
		}
